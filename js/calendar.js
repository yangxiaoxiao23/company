...
!function (window, document) {
	 ...
	 *	{string} dateFormat : 日期格式(可选) 默认为YYYY-MM-DD，可选的有MM/DD/YYYY、DD/MM/YYYY、MM-DD-YYYY、DD-MM-YYYY
	 *	{array} weekName : 星期的表示方式数组(可选) 默认为["周日", "周一", "周二", "周三", "周四", "周五", "周六"]
	 * @return {object}
	 */
	var Calendar = window.Calendar = function (options) {
		this.init(options || {});
		this.createFrame();
	},
	...
	Calendar.prototype = {
		...
		createFrame : function () {
			...
			try {
				i = this.iframe = document.createElement("<iframe src='about:blank' frameborder='0';></iframe>");
				wrap.appendChild(i);
			} catch (e) {}
