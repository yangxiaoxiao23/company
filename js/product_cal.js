...
L_calendar.prototype={
    HelpMsg:"1.蓝色框日期为普通可出发日期;<br>2.粉红背景红框为特别价格日期;<br>3.红色框日期为今日日期;<br>4.灰色日期没有行程安排,不可选;<br>5.被加中划线是已卖完的日期;<br>6.日历底部文字是对日期的说明",/* 在这里添加和修改帮助文档的文字 */
    ...
    CreateHTML:function(){
        ...
        htmlstr+="<div id=\"L_calendar-PrevM\" onclick=\"parent."+this.NewName+".PrevM()\" title=\"前一月\"><b>&lt;</b><span id=\"L_calendar-PrevM-text\"></span></div>\r\n";
        ...
        htmlstr+="<div id=\"L_calendar-NextM\" onclick=\"parent."+this.NewName+".NextM()\" title=\"后一月\"><span id=\"L_calendar-NextM-text\"></span><b>&gt;</b></div>\r\n";
        htmlstr+="</div>\r\n";
        htmlstr+="<div id=\"L_calendar-week\"><ul  onmouseup=\"StopMove()\"><li><b>日</b></li><li>一</li><li>二</li><li>三</li><li>四</li><li>五</li><li><b>六</b></li></ul></div>\r\n";
        ...
        htmlstr+="</div>\r\n";
        htmlstr+="<div>\r\n";
        htmlstr+="<div id=\"L_calendar-help\" onclick='sAlert(\""+this.HelpMsg+"\")'><span>帮助</span></div>\r\n";
        ...
        htmlstr+="<div id=\"L_calendar-close\" onclick='parent."+this.NewName+".OnClose()'><span>关闭</span></div>\r\n";
        htmlstr+="</div>\r\n";
        htmlstr+="</div>\r\n";
    ...
    WriteHead:function (yy,mm)  /* 往 head 中写入当前的年与月 */
    {
        ...
        this.InsertHTML("L_calendar-PrevM-text",prevM + "月");
        this.InsertHTML("L_calendar-NextM-text",nextM + "月");
    },
    ...
    DayClick:function(mm,dd,ynJQ,info,selectIndex)  /* 点击显示框选取日期，主输入函数************* */
    {
        ...
        if (this.ClickObject){
            ...
            if(ynJQ==false){
                ...
                if(info=="(假日)")
                {
                    this.InputObject.value= mm+"/"+dd+"/"+yy+" ("+this.GetDOW(dd,mm,yy)+") (假日价格)";
                }
                else
                {
    ...
    SetDate:function(){
		...
		if(this.DayInfo.length==0){
            ...
            if(availabletour_date==null){ alert("日期下拉列表不存在！请与管理员联系！"); }
			var aDateCount=availabletour_date.length;
            var nowMonth= new Date().getMonth()+1;
            var todayString=new Date().getFullYear()+"-"+nowMonth+"-"+new Date().getDate();

			if(typeof(availabletour_date.options[1])=="undefined"){ alert("无出发日期，该团已经卖完。"); return false; }
...
"+comp_now_date); */
                ...
                if(comp_temp_date==comp_now_date){
                    ...
                    if(dateOptionValue.indexOf("!!!")>-1){
                        ...
                        {
                            this.DayInfo[i]=new Array(true,true,"(假日)",j);
                        }
                    }
                    j++;
                    Time_difference = 0;
...
function AddToCart(){
	...
	if(departureDateCheck && form.elements["availabletourdate"].value.length < 4 ){
		...
		/* alert("TEXT_SELECT_DEPARTURE_DATE");	//选择出发日期 */
		/* return false; */
	...
	for(i=0; i<form.length; i++){      
		...
		if(sparam!="")aparams.push(sparam);   /* push是把新元素添加到阵列中去 */
	}    
...
var maxPerRoomPeopleNum = 4;	/* 该产品的每个房间最大人数 */
/*  去掉成人数和儿童数的多余的选项 */
	...
	if(numberOfRooms!==undefined){
		...
		for(i=0; i<room_num; i++){
			...
			if(bed_select===undefined){
				...
				options_array[0] = new Array(0,'不限制');
				options_array[1] = new Array(1,'一张King-sized大床');
				options_array[2] = new Array(2,'两张标准床');
...
function renderRoomSelect() {
	...
	for (var i = 1; i < 17; i++) {
		if(i==16){
						x += '<option value="'+i+'"'+(numRooms == i ? ' selected' : '')+'>' + '结伴拼房';
					}else{
...
function renderRoomSelectForTravelCompanion(){
    ...
    x += '<option value="0">不选择';
        //x += '<option value="1"'+(numRooms == 1 ? ' selected' : '')+'>' + '结伴同游';
    x += '<option value="1">' + '结伴同游';
		...
		ajax.onreadystatechange = function(){
			if (ajax.readyState == 4 && ajax.status == 200){
				...
				if(ajax.responseText.search(/book_now_out/)!=-1){
					var error_msn = "该团剩余座位小于订购人数,请选其它日期的团";
...
function SetAsGoodComment(reviews_id, good_or_bad){
	...
	function SetShowSteps2(products_options_id){
		...
		jQuery("#ConTitleA_ProductsOptions"+products_options_id).html('可修改');
	...
	function SetShowDepartureLocationsBox(){
		...
		jQuery("#ConTitleA_departurelocations").html('可修改');
		auto_update_budget();
	...
	function SetShowSteps3(){
		...
		if(jQuery("#ConTitle_hot-search-params h2").text() == "房间人数：选择"){
			addRoomTitle = true ;
		}
		...
		for(var i=0; i<loop_num; i++){
			...
			Steps3Html +='<dd><label>成人： </label>'+ From.elements["room-" +i+ "-adult-total"].value +'人</dd>';		
			
			if(From.elements["room-" +i+ "-child-total"].value > 0){ 					
				Steps3Html +='<dd><label>小孩: </label>'+ From.elements["room-" +i+ "-child-total"].value +'人</dd>';			
			 }
			if(typeof(From.elements["room-" +i+ "-bed"])!="undefined"){
				Steps3Html +='<dd class="size"><label>床型: </label>'+ jQuery(From.elements["room-" +i+ "-bed"]).find("option:selected").text() +'</dd>';
			}
		...
		jQuery("#ConTitleA_hot-search-params").html('可修改');
